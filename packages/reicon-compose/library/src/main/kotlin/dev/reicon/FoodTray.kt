// Auto-generated by tool/build_kotlin.py — do not edit.
package dev.reicon

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.PathFillType
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.addPathNodes
import androidx.compose.ui.unit.dp

object FoodTray {
    val Outline: ImageVector
        get() {
            if (_outline != null) return _outline!!
            _outline = ImageVector.Builder(
                name = "FoodTrayOutline",
                defaultWidth = 24.dp,
                defaultHeight = 24.dp,
                viewportWidth = 24f,
                viewportHeight = 24f
            ).apply {
                addPath(
                    pathData = addPathNodes("M15.4561 9.42422C15.1273 9.1723 14.6565 9.23463 14.4046 9.56344C14.1527 9.89224 14.215 10.363 14.5438 10.6149C15.1139 11.0517 15.5741 11.632 15.8722 12.304C16.0401 12.6827 16.4832 12.8535 16.8619 12.6855C17.2405 12.5176 17.4113 12.0745 17.2434 11.6958C16.8433 10.794 16.2252 10.0134 15.4561 9.42422Z"),
                    pathFillType = PathFillType.NonZero,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
                addPath(
                    pathData = addPathNodes("M14.25 4.5C14.25 4.84862 14.1707 5.17873 14.0292 5.47327C18.1617 6.39822 21.25 10.0885 21.25 14.5V16.25H21.2668C21.3685 16.25 21.4788 16.25 21.5746 16.2565C21.6826 16.2639 21.8258 16.282 21.9784 16.3452C22.2846 16.472 22.528 16.7154 22.6549 17.0217C22.718 17.1742 22.7361 17.3175 22.7435 17.4254C22.75 17.5212 22.75 17.6315 22.75 17.7332L22.75 17.774C22.75 18.103 22.75 18.3868 22.734 18.6218C22.7171 18.8692 22.68 19.1166 22.5787 19.361C22.3504 19.9124 21.9124 20.3504 21.361 20.5787C21.1166 20.68 20.8692 20.7171 20.6218 20.734C20.3869 20.75 20.103 20.75 19.7741 20.75H4.22601C3.89706 20.75 3.61314 20.75 3.37822 20.734C3.13085 20.7171 2.88341 20.68 2.63897 20.5787C2.08765 20.3504 1.64964 19.9124 1.42128 19.361C1.32002 19.1166 1.28289 18.8692 1.26602 18.6218C1.24999 18.3868 1.24999 18.103 1.25 17.774L1.25 17.7332C1.24998 17.6315 1.24996 17.5213 1.2565 17.4254C1.26386 17.3175 1.28196 17.1742 1.34516 17.0217C1.47202 16.7154 1.71537 16.472 2.02165 16.3452C2.17421 16.282 2.31746 16.2639 2.42537 16.2565C2.52124 16.25 2.63152 16.25 2.73319 16.25L2.75 16.25L2.75 14.5C2.75 10.0885 5.83827 6.39822 9.97082 5.47327C9.82929 5.17873 9.75 4.84862 9.75 4.5C9.75 3.25736 10.7574 2.25 12 2.25C13.2426 2.25 14.25 3.25736 14.25 4.5ZM12 3.75C11.5858 3.75 11.25 4.08579 11.25 4.5C11.25 4.91421 11.5858 5.25 12 5.25C12.4142 5.25 12.75 4.91421 12.75 4.5C12.75 4.08579 12.4142 3.75 12 3.75ZM4.25 14.5C4.25 10.2198 7.71979 6.75 12 6.75C16.2802 6.75 19.75 10.2198 19.75 14.5V16.25H4.25V14.5ZM21.2375 18.5197C21.2496 18.3419 21.25 18.1097 21.25 17.75H2.75C2.75 18.1097 2.75041 18.3419 2.76254 18.5197C2.7742 18.6907 2.79417 18.7558 2.80709 18.787C2.88322 18.9708 3.02922 19.1168 3.21299 19.1929C3.2442 19.2058 3.30933 19.2258 3.48032 19.2375C3.65806 19.2496 3.89029 19.25 4.25 19.25H19.75C20.1097 19.25 20.3419 19.2496 20.5197 19.2375C20.6907 19.2258 20.7558 19.2058 20.787 19.1929C20.9708 19.1168 21.1168 18.9708 21.1929 18.787C21.2058 18.7558 21.2258 18.6907 21.2375 18.5197Z"),
                    pathFillType = PathFillType.EvenOdd,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
            }.build()
            return _outline!!
        }

    private var _outline: ImageVector? = null

    val Filled: ImageVector
        get() {
            if (_filled != null) return _filled!!
            _filled = ImageVector.Builder(
                name = "FoodTrayFilled",
                defaultWidth = 24.dp,
                defaultHeight = 24.dp,
                viewportWidth = 24f,
                viewportHeight = 24f
            ).apply {
                addPath(
                    pathData = addPathNodes("M14.0292 5.47327C14.1707 5.17873 14.25 4.84862 14.25 4.5C14.25 3.25736 13.2426 2.25 12 2.25C10.7574 2.25 9.75 3.25736 9.75 4.5C9.75 4.84862 9.82929 5.17873 9.97082 5.47327C5.83827 6.39822 2.75 10.0885 2.75 14.5C2.75 14.9142 3.08579 15.25 3.5 15.25H20.5C20.9142 15.25 21.25 14.9142 21.25 14.5C21.25 10.0885 18.1617 6.39822 14.0292 5.47327ZM12 3.75C11.5858 3.75 11.25 4.08579 11.25 4.5C11.25 4.91421 11.5858 5.25 12 5.25C12.4142 5.25 12.75 4.91421 12.75 4.5C12.75 4.08579 12.4142 3.75 12 3.75ZM14.3307 8.63686C14.5651 8.29533 15.032 8.20844 15.3735 8.4428C16.1723 8.99096 16.8305 9.73808 17.2772 10.6178C17.4647 10.9871 17.3173 11.4385 16.948 11.6261C16.5787 11.8136 16.1273 11.6662 15.9397 11.2969C15.6069 10.6414 15.1169 10.0859 14.5248 9.67962C14.1833 9.44526 14.0964 8.9784 14.3307 8.63686Z"),
                    pathFillType = PathFillType.EvenOdd,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
                addPath(
                    pathData = addPathNodes("M2 16.5C1.72386 16.5 1.5 16.7239 1.5 17V17.2211C1.49999 17.6932 1.49999 18.0778 1.5255 18.3901C1.55187 18.7128 1.60797 19.0021 1.74524 19.2715C1.96095 19.6948 2.30516 20.039 2.72852 20.2548C2.99793 20.392 3.28715 20.4481 3.60989 20.4745C3.92215 20.5 4.30678 20.5 4.77893 20.5H19.2211C19.6932 20.5 20.0778 20.5 20.3901 20.4745C20.7128 20.4481 21.0021 20.392 21.2715 20.2548C21.6948 20.039 22.039 19.6948 22.2548 19.2715C22.392 19.0021 22.4481 18.7128 22.4745 18.3901C22.5 18.0778 22.5 17.6932 22.5 17.2211V17C22.5 16.7239 22.2761 16.5 22 16.5H2Z"),
                    pathFillType = PathFillType.NonZero,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
            }.build()
            return _filled!!
        }

    private var _filled: ImageVector? = null
}
