// Auto-generated by tool/build_kotlin.py — do not edit.
package dev.reicon

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.PathFillType
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.StrokeCap
import androidx.compose.ui.graphics.StrokeJoin
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.addPathNodes
import androidx.compose.ui.unit.dp

object Dropper4 {
    val Outline: ImageVector
        get() {
            if (_outline != null) return _outline!!
            _outline = ImageVector.Builder(
                name = "Dropper4Outline",
                defaultWidth = 24.dp,
                defaultHeight = 24.dp,
                viewportWidth = 24f,
                viewportHeight = 24f
            ).apply {
                addPath(
                    pathData = addPathNodes("M19 8L17 8"),
                    pathFillType = PathFillType.NonZero,
                    fill = null,
                    stroke = SolidColor(Color.Black),
                    strokeLineWidth = 1.5f,
                    strokeLineCap = StrokeCap.Round,
                    strokeLineJoin = StrokeJoin.Round
                )
                addPath(
                    pathData = addPathNodes("M19 14H17"),
                    pathFillType = PathFillType.NonZero,
                    fill = null,
                    stroke = SolidColor(Color.Black),
                    strokeLineWidth = 1.5f,
                    strokeLineCap = StrokeCap.Round,
                    strokeLineJoin = StrokeJoin.Round
                )
                addPath(
                    pathData = addPathNodes("M19 11H17"),
                    pathFillType = PathFillType.NonZero,
                    fill = null,
                    stroke = SolidColor(Color.Black),
                    strokeLineWidth = 1.5f,
                    strokeLineCap = StrokeCap.Round,
                    strokeLineJoin = StrokeJoin.Round
                )
                addPath(
                    pathData = addPathNodes("M13.1752 9.10145C12.5324 8.42322 11.4676 8.42322 10.8248 9.10145C10.5172 9.42597 10.1419 9.85664 9.83883 10.3157C9.5544 10.7466 9.25 11.3242 9.25 11.9166C9.25 13.4523 10.4527 14.7499 12 14.7499C13.5473 14.7499 14.75 13.4523 14.75 11.9166C14.75 11.3242 14.4456 10.7466 14.1612 10.3157C13.8581 9.85664 13.4828 9.42597 13.1752 9.10145ZM11.9135 10.1333C11.9428 10.1024 11.9734 10.0928 12 10.0928C12.0266 10.0928 12.0572 10.1024 12.0865 10.1333C12.3651 10.4272 12.6744 10.7862 12.9093 11.1421C13.1629 11.5263 13.25 11.7891 13.25 11.9166C13.25 12.6821 12.6618 13.2499 12 13.2499C11.3382 13.2499 10.75 12.6821 10.75 11.9166C10.75 11.7891 10.8371 11.5263 11.0907 11.1421C11.3256 10.7862 11.6349 10.4272 11.9135 10.1333Z"),
                    pathFillType = PathFillType.EvenOdd,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
                addPath(
                    pathData = addPathNodes("M9.35352 3.25C9.67998 2.09575 10.7412 1.25 12 1.25C13.2588 1.25 14.32 2.09575 14.6465 3.25L15.052 3.25C15.9505 3.24997 16.6997 3.24994 17.2945 3.32991C17.9223 3.41432 18.4891 3.59999 18.9445 4.05546C19.4 4.51093 19.5857 5.07773 19.6701 5.70552C19.7501 6.3003 19.75 7.04951 19.75 7.94797L19.75 15.8831C19.75 16.8539 19.3428 17.7803 18.6274 18.4367C16.9503 19.9756 14.8713 20.826 12.7499 20.988L12.75 21V22C12.75 22.4142 12.4142 22.75 12 22.75C11.5858 22.75 11.25 22.4142 11.25 22V21L11.2501 20.988C9.12873 20.826 7.04967 19.9756 5.37259 18.4367C4.65724 17.7803 4.25 16.8539 4.25 15.8831L4.25 7.948C4.24997 7.04953 4.24995 6.30029 4.32991 5.70552C4.41432 5.07773 4.59999 4.51093 5.05546 4.05546C5.51093 3.59999 6.07773 3.41432 6.70552 3.32991C7.3003 3.24994 8.04952 3.24997 8.948 3.25L9.35352 3.25ZM10.9999 3.25C11.228 2.94639 11.591 2.75 12 2.75C12.409 2.75 12.772 2.94639 13.0001 3.25H10.9999ZM9 4.75C8.03599 4.75 7.38843 4.75159 6.9054 4.81654C6.44393 4.87858 6.24644 4.9858 6.11612 5.11612C5.9858 5.24643 5.87858 5.44393 5.81654 5.90539C5.7516 6.38843 5.75 7.03599 5.75 8V15.8831C5.75 16.4337 5.98098 16.9592 6.38673 17.3315C9.56185 20.2449 14.4382 20.2449 17.6133 17.3315C18.019 16.9592 18.25 16.4337 18.25 15.8831V8C18.25 7.03599 18.2484 6.38843 18.1835 5.90539C18.1214 5.44393 18.0142 5.24643 17.8839 5.11612C17.7536 4.9858 17.5561 4.87858 17.0946 4.81654C16.6116 4.75159 15.964 4.75 15 4.75H9Z"),
                    pathFillType = PathFillType.EvenOdd,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
            }.build()
            return _outline!!
        }

    private var _outline: ImageVector? = null

    val Filled: ImageVector
        get() {
            if (_filled != null) return _filled!!
            _filled = ImageVector.Builder(
                name = "Dropper4Filled",
                defaultWidth = 24.dp,
                defaultHeight = 24.dp,
                viewportWidth = 24f,
                viewportHeight = 24f
            ).apply {
                addPath(
                    pathData = addPathNodes("M19 11.75V13.25H17C16.5858 13.25 16.25 13.5858 16.25 14C16.25 14.4142 16.5858 14.75 17 14.75H19V15.8831C19 16.6438 18.6809 17.3697 18.1203 17.8841C16.5867 19.2913 14.6891 20.0751 12.75 20.2356V21.25C12.75 21.6642 12.4142 22 12 22C11.5858 22 11.25 21.6642 11.25 21.25V20.2356C9.31089 20.0751 7.41327 19.2913 5.87966 17.8841C5.31911 17.3697 5 16.6438 5 15.8831V8C5 6.11438 5 5.17157 5.58579 4.58579C6.17157 4 7.11438 4 9 4H15C16.8856 4 17.8284 4 18.4142 4.58579C18.9166 5.08814 18.9881 5.85306 18.9983 7.25H17C16.5858 7.25 16.25 7.58579 16.25 8C16.25 8.41421 16.5858 8.75 17 8.75H19V10.25H17C16.5858 10.25 16.25 10.5858 16.25 11C16.25 11.4142 16.5858 11.75 17 11.75H19ZM12 14C13.1046 14 14 13.0673 14 11.9167C14 11.1968 13.217 10.2359 12.6309 9.61744C12.2839 9.25132 11.7161 9.25132 11.3691 9.61744C10.783 10.2359 10 11.1968 10 11.9167C10 13.0673 10.8954 14 12 14Z"),
                    pathFillType = PathFillType.EvenOdd,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
                addPath(
                    pathData = addPathNodes("M13.7325 3C13.3866 2.4022 12.7403 2 12 2C11.2597 2 10.6134 2.4022 10.2676 3L13.7325 3Z"),
                    pathFillType = PathFillType.NonZero,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
            }.build()
            return _filled!!
        }

    private var _filled: ImageVector? = null
}
