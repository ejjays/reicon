// Auto-generated by tool/build_kotlin.py — do not edit.
package dev.reicon

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.PathFillType
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.addPathNodes
import androidx.compose.ui.unit.dp

object Traffic2 {
    val Outline: ImageVector
        get() {
            if (_outline != null) return _outline!!
            _outline = ImageVector.Builder(
                name = "Traffic2Outline",
                defaultWidth = 24.dp,
                defaultHeight = 24.dp,
                viewportWidth = 24f,
                viewportHeight = 24f
            ).apply {
                addPath(
                    pathData = addPathNodes("M9.90278 22.5434C11.9881 22.9582 14.1495 22.7453 16.1139 21.9317C18.0782 21.1181 19.7571 19.7402 20.9383 17.9724C22.1195 16.2046 22.75 14.1262 22.75 12C22.75 11.5858 22.4142 11.25 22 11.25C21.5858 11.25 21.25 11.5858 21.25 12C21.25 13.8295 20.7075 15.6179 19.6911 17.139C18.6747 18.6602 17.23 19.8458 15.5398 20.5459C13.8496 21.246 11.9897 21.4292 10.1954 21.0723C8.4011 20.7154 6.7529 19.8344 5.45927 18.5407C4.16563 17.2471 3.28465 15.5989 2.92774 13.8046C2.57083 12.0103 2.75401 10.1504 3.45412 8.46018C4.15423 6.76996 5.33983 5.32531 6.86098 4.30891C8.38213 3.2925 10.1705 2.75 12 2.75C12.4142 2.75 12.75 2.41421 12.75 2C12.75 1.58579 12.4142 1.25 12 1.25C9.87386 1.25 7.79545 1.88048 6.02763 3.0617C4.2598 4.24293 2.88194 5.92185 2.0683 7.88615C1.25466 9.85046 1.04177 12.0119 1.45656 14.0972C1.87135 16.1825 2.89519 18.098 4.39861 19.6014C5.90202 21.1048 7.81749 22.1287 9.90278 22.5434Z"),
                    pathFillType = PathFillType.NonZero,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
                addPath(
                    pathData = addPathNodes("M12 8.25C12.4142 8.25 12.75 8.58579 12.75 9V11.25H15C15.4142 11.25 15.75 11.5858 15.75 12C15.75 12.4142 15.4142 12.75 15 12.75H12.75L12.75 15C12.75 15.4142 12.4142 15.75 12 15.75C11.5858 15.75 11.25 15.4142 11.25 15V12.75H9.00001C8.58579 12.75 8.25001 12.4142 8.25001 12C8.25001 11.5858 8.58579 11.25 9.00001 11.25H11.25L11.25 9C11.25 8.58579 11.5858 8.25 12 8.25Z"),
                    pathFillType = PathFillType.NonZero,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
                addPath(
                    pathData = addPathNodes("M14.6869 1.58861C14.2858 1.48537 13.8769 1.72686 13.7737 2.128C13.6704 2.52914 13.9119 2.93802 14.3131 3.04127C17.5625 3.8776 20.1223 6.43745 20.9586 9.68684C21.0619 10.088 21.4708 10.3295 21.8719 10.2262C22.273 10.123 22.5145 9.71409 22.4113 9.31295C21.4387 5.5343 18.4656 2.56117 14.6869 1.58861Z"),
                    pathFillType = PathFillType.NonZero,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
            }.build()
            return _outline!!
        }

    private var _outline: ImageVector? = null

    val Filled: ImageVector
        get() {
            if (_filled != null) return _filled!!
            _filled = ImageVector.Builder(
                name = "Traffic2Filled",
                defaultWidth = 24.dp,
                defaultHeight = 24.dp,
                viewportWidth = 24f,
                viewportHeight = 24f
            ).apply {
                addPath(
                    pathData = addPathNodes("M3.46447 20.5355C4.92893 22 7.28595 22 12 22C16.714 22 19.0711 22 20.5355 20.5355C22 19.0711 22 16.714 22 12C22 7.28595 22 4.92893 20.5355 3.46447C19.0711 2 16.714 2 12 2C7.28595 2 4.92893 2 3.46447 3.46447C2 4.92893 2 7.28595 2 12C2 16.714 2 19.0711 3.46447 20.5355ZM14.5831 18.2362C13.3497 18.7471 11.9925 18.8808 10.6831 18.6203C9.37377 18.3599 8.17103 17.717 7.22703 16.773C6.28303 15.829 5.64015 14.6262 5.3797 13.3169C5.11925 12.0075 5.25292 10.6503 5.76381 9.41689C6.2747 8.18349 7.13987 7.12928 8.2499 6.38758C9.35993 5.64588 10.665 5.25 12 5.25C12.4142 5.25 12.75 5.58579 12.75 6C12.75 6.41421 12.4142 6.75 12 6.75C10.9616 6.75 9.94662 7.05791 9.08326 7.63479C8.2199 8.21166 7.54699 9.0316 7.14963 9.99091C6.75227 10.9502 6.64831 12.0058 6.85088 13.0242C7.05345 14.0426 7.55346 14.9781 8.28769 15.7123C9.02192 16.4465 9.95738 16.9466 10.9758 17.1491C11.9942 17.3517 13.0498 17.2477 14.0091 16.8504C14.9684 16.453 15.7883 15.7801 16.3652 14.9167C16.9421 14.0534 17.25 13.0384 17.25 12C17.25 11.5858 17.5858 11.25 18 11.25C18.4142 11.25 18.75 11.5858 18.75 12C18.75 13.335 18.3541 14.6401 17.6124 15.7501C16.8707 16.8601 15.8165 17.7253 14.5831 18.2362ZM14.2957 5.65159C13.915 5.48837 13.4741 5.66467 13.3109 6.04536C13.1476 6.42606 13.3239 6.867 13.7046 7.03022C15.1679 7.65761 16.3424 8.83208 16.9698 10.2954C17.133 10.6761 17.5739 10.8524 17.9546 10.6891C18.3353 10.5259 18.5116 10.085 18.3484 9.70428C17.5693 7.88714 16.1129 6.4307 14.2957 5.65159ZM12 9.74995C12.4142 9.74995 12.75 10.0857 12.75 10.5V11.25H13.5C13.9142 11.25 14.25 11.5857 14.25 12C14.25 12.4142 13.9142 12.75 13.5 12.75H12.75V13.5C12.75 13.9142 12.4142 14.25 12 14.25C11.5858 14.25 11.25 13.9142 11.25 13.5V12.75H10.5C10.0858 12.75 9.75 12.4142 9.75 12C9.75 11.5857 10.0858 11.25 10.5 11.25H11.25V10.5C11.25 10.0857 11.5858 9.74995 12 9.74995Z"),
                    pathFillType = PathFillType.EvenOdd,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
            }.build()
            return _filled!!
        }

    private var _filled: ImageVector? = null
}
