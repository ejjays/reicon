// Auto-generated by tool/build_kotlin.py — do not edit.
package dev.reicon

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.PathFillType
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.addPathNodes
import androidx.compose.ui.unit.dp

object UploadSquare2 {
    val Outline: ImageVector
        get() {
            if (_outline != null) return _outline!!
            _outline = ImageVector.Builder(
                name = "UploadSquare2Outline",
                defaultWidth = 24.dp,
                defaultHeight = 24.dp,
                viewportWidth = 24f,
                viewportHeight = 24f
            ).apply {
                addPath(
                    pathData = addPathNodes("M5.25 7C5.25 7.41421 5.58579 7.75 6 7.75H18C18.4142 7.75 18.75 7.41421 18.75 7C18.75 6.58579 18.4142 6.25 18 6.25H6C5.58579 6.25 5.25 6.58579 5.25 7Z"),
                    pathFillType = PathFillType.NonZero,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
                addPath(
                    pathData = addPathNodes("M9.25 17C9.25 17.4142 8.91421 17.75 8.5 17.75C8.08579 17.75 7.75 17.4142 7.75 17V12.0715L6.57617 13.4801C6.31099 13.7983 5.83807 13.8413 5.51986 13.5762C5.20165 13.311 5.15866 12.8381 5.42383 12.5199L7.92383 9.51986C8.06633 9.34887 8.27742 9.25 8.5 9.25C8.72259 9.25 8.93367 9.34887 9.07617 9.51986L11.5762 12.5199C11.8413 12.8381 11.7983 13.311 11.4801 13.5762C11.1619 13.8413 10.689 13.7983 10.4238 13.4801L9.25 12.0715V17Z"),
                    pathFillType = PathFillType.NonZero,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
                addPath(
                    pathData = addPathNodes("M16.25 17C16.25 17.4142 15.9142 17.75 15.5 17.75C15.0858 17.75 14.75 17.4142 14.75 17V12.0715L13.5762 13.4801C13.311 13.7983 12.8381 13.8413 12.5199 13.5762C12.2017 13.311 12.1587 12.8381 12.4238 12.5199L14.9238 9.51986C15.0663 9.34887 15.2774 9.25 15.5 9.25C15.7226 9.25 15.9337 9.34887 16.0762 9.51986L18.5762 12.5199C18.8413 12.8381 18.7983 13.311 18.4801 13.5762C18.1619 13.8413 17.689 13.7983 17.4238 13.4801L16.25 12.0715V17Z"),
                    pathFillType = PathFillType.NonZero,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
                addPath(
                    pathData = addPathNodes("M11.9426 1.25C9.63423 1.24999 7.82519 1.24998 6.4137 1.43975C4.96897 1.63399 3.82895 2.03933 2.93414 2.93414C2.03933 3.82895 1.63399 4.96897 1.43975 6.41371C1.24998 7.82519 1.24999 9.63423 1.25 11.9426V12.0574C1.24999 14.3658 1.24998 16.1748 1.43975 17.5863C1.63399 19.031 2.03933 20.1711 2.93414 21.0659C3.82895 21.9607 4.96897 22.366 6.4137 22.5603C7.82519 22.75 9.63423 22.75 11.9426 22.75H12.0574C14.3658 22.75 16.1748 22.75 17.5863 22.5603C19.031 22.366 20.1711 21.9607 21.0659 21.0659C21.9607 20.1711 22.366 19.031 22.5603 17.5863C22.75 16.1748 22.75 14.3658 22.75 12.0574V11.9426C22.75 9.63423 22.75 7.82519 22.5603 6.41371C22.366 4.96897 21.9607 3.82895 21.0659 2.93414C20.1711 2.03933 19.031 1.63399 17.5863 1.43975C16.1748 1.24998 14.3658 1.24999 12.0574 1.25H11.9426ZM3.9948 3.9948C4.56445 3.42514 5.33517 3.09825 6.61358 2.92637C7.91356 2.75159 9.62177 2.75 12 2.75C14.3782 2.75 16.0864 2.75159 17.3864 2.92637C18.6648 3.09825 19.4355 3.42514 20.0052 3.9948C20.5749 4.56445 20.9018 5.33517 21.0736 6.61358C21.2484 7.91356 21.25 9.62178 21.25 12C21.25 14.3782 21.2484 16.0864 21.0736 17.3864C20.9018 18.6648 20.5749 19.4355 20.0052 20.0052C19.4355 20.5749 18.6648 20.9018 17.3864 21.0736C16.0864 21.2484 14.3782 21.25 12 21.25C9.62177 21.25 7.91356 21.2484 6.61358 21.0736C5.33517 20.9018 4.56445 20.5749 3.9948 20.0052C3.42514 19.4355 3.09825 18.6648 2.92637 17.3864C2.75159 16.0864 2.75 14.3782 2.75 12C2.75 9.62178 2.75159 7.91356 2.92637 6.61358C3.09825 5.33517 3.42514 4.56445 3.9948 3.9948Z"),
                    pathFillType = PathFillType.EvenOdd,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
            }.build()
            return _outline!!
        }

    private var _outline: ImageVector? = null

    val Filled: ImageVector
        get() {
            if (_filled != null) return _filled!!
            _filled = ImageVector.Builder(
                name = "UploadSquare2Filled",
                defaultWidth = 24.dp,
                defaultHeight = 24.dp,
                viewportWidth = 24f,
                viewportHeight = 24f
            ).apply {
                addPath(
                    pathData = addPathNodes("M3.46447 3.46447C2 4.92893 2 7.28595 2 12C2 16.714 2 19.0711 3.46447 20.5355C4.92893 22 7.28595 22 12 22C16.714 22 19.0711 22 20.5355 20.5355C22 19.0711 22 16.714 22 12C22 7.28595 22 4.92893 20.5355 3.46447C19.0711 2 16.714 2 12 2C7.28595 2 4.92893 2 3.46447 3.46447ZM9.25 17C9.25 17.4142 8.91421 17.75 8.5 17.75C8.08579 17.75 7.75 17.4142 7.75 17V12.0715L6.57617 13.4801C6.31099 13.7983 5.83807 13.8413 5.51986 13.5762C5.20165 13.311 5.15866 12.8381 5.42383 12.5199L7.92383 9.51986C8.06633 9.34887 8.27742 9.25 8.5 9.25C8.72259 9.25 8.93367 9.34887 9.07617 9.51986L11.5762 12.5199C11.8413 12.8381 11.7983 13.311 11.4801 13.5762C11.1619 13.8413 10.689 13.7983 10.4238 13.4801L9.25 12.0715V17ZM15.5 17.75C15.9142 17.75 16.25 17.4142 16.25 17V12.0715L17.4238 13.4801C17.689 13.7983 18.1619 13.8413 18.4801 13.5762C18.7983 13.311 18.8413 12.8381 18.5762 12.5199L16.0762 9.51986C15.9337 9.34887 15.7226 9.25 15.5 9.25C15.2774 9.25 15.0663 9.34887 14.9238 9.51986L12.4238 12.5199C12.1587 12.8381 12.2017 13.311 12.5199 13.5762C12.8381 13.8413 13.311 13.7983 13.5762 13.4801L14.75 12.0715V17C14.75 17.4142 15.0858 17.75 15.5 17.75ZM6 7.75C5.58579 7.75 5.25 7.41421 5.25 7C5.25 6.58579 5.58579 6.25 6 6.25H18C18.4142 6.25 18.75 6.58579 18.75 7C18.75 7.41421 18.4142 7.75 18 7.75H6Z"),
                    pathFillType = PathFillType.EvenOdd,
                    fill = SolidColor(Color.Black),
                    stroke = null
                )
            }.build()
            return _filled!!
        }

    private var _filled: ImageVector? = null
}
